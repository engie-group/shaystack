...
import haystackapi
import hszinc
from haystackapi import HaystackHttpRequest, DEFAULT_MIME_TYPE, Grid, VER_3_0
...
def test_hisWrite_with_zinc(mock):
    # GIVEN
...
@patch.dict('os.environ', {'HAYSTACK_PROVIDER': 'haystackapi.providers.ping'})
@patch.object(ping.Provider, 'his_write')
def test_hisWrite_with_args(mock):
    # GIVEN
    ts = [
    ...
    result_ts.extend([{"date": parse_date_format(d), "val": v} for d, v in ts])
