...
@pytest.fixture()
def apigw_event():
    with open('events/PointWrite_event.json') as json_file:
...
def test_pointWrite_with_zinc(apigw_event: LambdaProxyEvent):
    # GIVEN
    context = LambdaContext()
    context.function_name = "PointWrite"
...
def test_pointWrite(apigw_event: LambdaEvent, lambda_client: BaseClient) -> None:
    # GIVEN
    apigw_event["headers"]["Accept-Encoding"] = "gzip, deflate, sdch"
    # WHEN
    boto_response = lambda_client.invoke(
        FunctionName="PointWrite",
