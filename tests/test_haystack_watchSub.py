...
@pytest.fixture()
def apigw_event():
    with open('events/WatchSub_event.json') as json_file:
...
def test_watchSub_with_zinc(apigw_event: LambdaProxyEvent):
    # GIVEN
    context = LambdaContext()
    context.function_name = "WathSub"
    ...
    # WHEN
    response = haystackapi_lambda.watch_sub(apigw_event, context)
...
def test_watchSub(apigw_event: LambdaEvent, lambda_client: BaseClient) -> None:
    # GIVEN
    apigw_event["headers"]["Accept-Encoding"] = "gzip, deflate, sdch"
    # WHEN
    boto_response = lambda_client.invoke(
        FunctionName="WatchSub",
