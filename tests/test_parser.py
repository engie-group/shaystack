...
def _check_number(grid):
    ...
    row = grid.pop(0)
    assert row['number'] == haystackapi.Quantity(74.2, u'°F')
    row = grid.pop(0)
...
def test_unsupported_old_zinc():
    def _check_unsupported_old_zinc(pint_en):
        ...
        with warnings.catch_warnings(record=True):
            ...
            grid = haystackapi.parse('''ver:"1.0"
comment
...
def test_unsupported_newer_zinc():
    def _check_unsupported_newer_zinc(pint_en):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            grid = haystackapi.parse('''ver:"2.5"
comment
...
def test_unsupported_bleedingedge_zinc():
    def _check_unsupported_bleedingedge_zinc(pint_en):
        ...
        with warnings.catch_warnings(record=True):
            warnings.simplefilter("always")
            grid = haystackapi.parse('''ver:"9999.9999"
comment
...
def test_malformed_grid_zinc():
    def _check_malformed_grid_zinc(pint_en):
        _enable_pint(pint_en)
        try:
            haystackapi.parse('''ver:2.0 comment:"This grid has no columns!"
    ''')
...
def test_malformed_version_zinc():
    def _check_malformed_version_zinc(pint_en):
        _enable_pint(pint_en)
        try:
            haystackapi.parse('''ver:TwoPointOh comment:"This grid has an invalid version!"
    empty
    ''')
...
def test_remove_csv():
    def _check_remove_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''str,remove
"v2 REMOVE value",R
"v3 REMOVE value",R
''', mode=haystackapi.MODE_CSV)
        _check_remove(grid)
...
def test_marker_in_row_zinc():
    def _check_marker_in_row_zinc(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''ver:"2.0"
str,marker
"No Marker",
"Marker",M
''')
...
def test_marker_in_row_csv():
    def _check_marker_in_row_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse(u'''str,marker
"No Marker",
"Marker",\u2713
''', mode=haystackapi.MODE_CSV)
        assert 'marker' not in grid[0]
...
def test_bool_csv():
    def _check_bool_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''str,bool
"True",true
"False",false''', mode=haystackapi.MODE_CSV)
        assert grid[0]['bool']
        assert not grid[1]['bool']
...
def test_number_zinc():
    def _check_number_zinc(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse(u'''ver:"2.0"
str,number
"Integer",1
"Negative Integer",-34
...
def test_number_csv():
    def _check_number_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse(u'''str,number
"Integer",1
"Negative Integer",-34
"With Separators",10_000
"Scientific",5.4e-45
...
"Positive Infinity",INF
"Negative Infinity",-INF
"Not a Number",NaN''', mode=haystackapi.MODE_CSV)
        _check_number(grid)
...
def test_uri_csv():
    def _check_uri_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''uri
`http://www.vrt.com.au`''', mode=haystackapi.MODE_CSV)
        assert len(grid) == 1
        assert grid[0]['uri'] == haystackapi.Uri('http://www.vrt.com.au')
...
def test_ref_csv():
    def _check_ref_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''str,ref
"Basic",@a-basic-ref
"With value",@reference With value''', mode=haystackapi.MODE_CSV)
        assert len(grid) == 2
        assert grid[0]['ref'] == haystackapi.Ref('a-basic-ref')
        assert grid[1]['ref'] == haystackapi.Ref('reference', 'With value')
...
def test_date_csv():
    def _check_date_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''date
2010-03-13''', mode=haystackapi.MODE_CSV)
        assert len(grid) == 1
        assert isinstance(grid[0]['date'], datetime.date)
        assert grid[0]['date'] == datetime.date(2010, 3, 13)
...
def test_time_csv():
    def _check_time_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''time
08:12
08:12:05
08:12:05.5''', mode=haystackapi.MODE_CSV)
        assert len(grid) == 3
        row = grid.pop(0)
        assert isinstance(row['time'], datetime.time)
        assert row['time'] == datetime.time(8, 12)
...
def test_datetime_csv():
    def _check_datetime_csv(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''datetime
2010-11-28T07:23:02.500-08:00 Los_Angeles
2010-11-28T23:19:29.500+08:00 Taipei
2010-11-28T18:21:58+03:00 GMT-3
2010-11-28T12:22:27-03:00 GMT+3
2010-01-08T05:00:00Z UTC
2010-01-08T05:00:00Z''', mode=haystackapi.MODE_CSV)
        _check_datetime(grid)
...
def test_list_v3_csv():
    def _check_list_v3_csv(pint_en):
        ...
        # Simpler test case than the ZINC one, since the Python JSON parser
        # will take care of the elements for us.
        grid = haystackapi.parse(u'''ix,list,
00,"[]","An empty list"
01,"[N]","A list with a NULL"
02,"[T,F]","Booleans in a list"
03,"[1,2,3]","Integers"
...
def test_nodehaystack_02_zinc():
    def _check_nodehaystack_02_zinc(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''ver:"2.0" tag foo:"bar"
xyz
"val"
''')
        assert len(grid) == 1
...
''')
        ...
        row = grid.pop(0)
        assert row['a'] == haystackapi.Quantity(5, u'kWh/ft\u00b2')
        assert row['b'] == haystackapi.Quantity(-15, u'kWh/m\u00b2')
        row = grid.pop(0)
        assert row['a'] == haystackapi.Quantity(123e12, 'kJ/kg_dry')
        assert row['b'] == haystackapi.Quantity(74, u'\u0394\u00b0F')
...
def test_nodehaystack_09_zinc():
    def _check_nodehaystack_09_zinc(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse(u'''ver:"2.0" a: 2009-02-03T04:05:06Z foo b: 2010-02-03T04:05:06Z UTC bar c: 2009-12-03T04:05:06Z London baz
a
3.814697265625E-6
...
''')
        ...
        assert grid.pop(0)['a'] == haystackapi.Quantity(33, u'\u00a3')
        assert grid.pop(0)['a'] == haystackapi.Ref('12cbb08e-0c02ae73')
        assert grid.pop(0)['a'] == haystackapi.Quantity(7.15625E-4, u'kWh/ft\u00b2')
...
def test_nodehaystack_10_zinc():
    def _check_nodehaystack_10_zinc(pint_en):
        _enable_pint(pint_en)
        grid = haystackapi.parse('''ver:"2.0" bg: Bin(image/jpeg) mark
...
def test_malformed_grid_meta_zinc():
    def _check_malformed_grid_meta_zinc(pint_en):
        _enable_pint(pint_en)
        try:
            haystackapi.parse('''ver:"2.0" ThisIsNotATag
    empty
