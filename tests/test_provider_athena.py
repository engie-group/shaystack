...
class AthenaProviderTest(unittest.TestCase):
    ...
    def athena_client(self):
        with mock_athena():
            self.athena_client = boto3.client("athena", region_name="eu-west-1")  #self._env["AWS_REGION"]
            yield self.athena_client
    ...
    @pytest.fixture(scope="class", autouse=True)
    def shaystack_grid_from_time_series_csv_file_1(self, time_series_csv_file_1):
        """Shaystack Grid object created from csv time series"""
        reader = csv.DictReader(time_series_csv_file_1)
        self.history = Grid(columns=["ts", "val"])
        for row in reader:
            date_val = row['time']
            hs_values = {'value': row['value']}
            self.history.append({"ts": datetime.fromisoformat(date_val).replace(tzinfo=pytz.UTC),
                            "val": DBTSProvider._cast_timeserie_to_hs(str(list(hs_values.values())[0]), "float")})

    @pytest.fixture(scope="class", autouse=True)
    def shaystack_empty_grid(self):
        """Shaystack empty Grid object"""
        self.empty_grid = Grid(columns=["ts", "val"])
