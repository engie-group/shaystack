...
@pytest.fixture(scope="function", autouse=True)
def test_athena_client(test_environ):
    with mock_athena():
        test_athena_client = boto3.client("athena", region_name=test_environ["AWS_REGION"])
        yield test_athena_client
...
@pytest.fixture()
def test_time_series_csv_file_1():
...
@pytest.fixture()
def test_shaystack_grid_from_time_series_csv_file_1(test_time_series_csv_file_1):
    """Shaystack Grid object created from csv time series"""
    reader = csv.DictReader(test_time_series_csv_file_1)
...
@pytest.fixture()
def test_shaystack_empty_grid():
    """Shaystack empty Grid object"""
    return Grid(columns=["ts", "val"])


def test_import_ts_in_db(test_environ):
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
        with pytest.raises(NotImplementedError):
            assert provider._import_ts_in_db()


@patch('shaystack.providers.athena.Provider.get_query_results')
def test_get_query_results_called_by_poll_query_status(mock_get_query_execution, test_environ, test_athena_client):
    query = "SELECT stuff"
    location = "s3://bucket-name/prefix/"
    database = "database"
    # Start Query
    response = test_athena_client.start_query_execution(
        QueryString=query,
...
def test_poll_query_status_raise_exception_when_querye_failed(test_environ, test_athena_client):
    query = "SELECT stuff"
    location = "s3://bucket-name/prefix/"
    database = "database"
    # Start Query
...
def test_get_query_results_object_not_found(test_environ):
...
def test_build_athena_prediction_query_of_entity_00(test_environ, test_entity_00):
    ...
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
        assert athena_query == provider.build_athena_query(test_entity_00['hisURI'], date_range, None)


def test_build_athena_prediction_query_of_entity_01(test_environ, test_entity_01):
    ...
    date_range = (datetime(2021, 5, 1, 0, 0, tzinfo=pytz.UTC),
                  datetime(2021, 5, 4, 23, 59, 59, 999999, tzinfo=pytz.UTC))
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
        assert athena_query == provider.build_athena_query(test_entity_01['hisURI'], date_range, None)


def test_create_history_grid(test_environ, test_time_series_csv_file_1, test_entity_01, test_shaystack_grid_from_time_series_csv_file_1):
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
        reader = csv.DictReader(test_time_series_csv_file_1)
        history = provider.create_history_grid(reader, test_entity_01['hisURI'])
        assert history == test_shaystack_grid_from_time_series_csv_file_1


def test_create_history_empty_grid(test_environ, test_shaystack_empty_grid, test_entity_01):
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
        history = provider.create_history_grid(None, test_entity_01['hisURI'])
        assert history == test_shaystack_empty_grid


def test_put_date_format(test_environ):
    str_date = "2022/06/01"
    date_pattern = "%Y/%m/%d"
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
...
def test_put_date_format_value_error_exception(test_environ):
    str_date = "2022/06/01"
    date_pattern = "%Y-%m-%d"
    with cast(DBTSProvider, get_provider("shaystack.providers.athena", test_environ)) as provider:
